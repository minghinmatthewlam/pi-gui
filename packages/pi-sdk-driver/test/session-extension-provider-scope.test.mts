...
/**
 * Sessions must resolve models against the runtime `createAgentSessionServices`
 * builds for their own cwd. These cover what a shared, cross-workspace runtime
 ...
 * `AgentSessionRuntime` around so a test can inspect the runtime the session
...
test("a session keeps its own workspace's endpoint and key for a shared provider and model id", async () => {
  ...
  const modelRuntime = runtimeFor(workspaceA.path);
  const model = modelRuntime.getModel("shared-id", "model-x");
  ...
  assert.equal(auth?.auth.apiKey, KEY_A, "workspace A's session must use workspace A's credential");
  await driver.closeSession(snapshot.ref);
});
...
`,
  ...
  assert.ok(
    runtimeSnapshot.models.some(
      (model) => model.providerId === "merge-id" && model.modelId === "model-a",
    ),
