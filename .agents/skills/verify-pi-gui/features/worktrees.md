...
- **Visible maintenance:** `.agents/skills/verify-pi-gui/scripts/prove.sh --maintenance` uses a disposable Git repository (never the user's checkout). It clicks `Workspace actions for ${name}`, then `Create permanent worktree`, opens New thread, and requires exact Local and Worktree buttons.
