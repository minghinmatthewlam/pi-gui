...
- `navigation-recency`: date buckets (Today / Last 7 Days / Last 30 Days / Older) across folders; send bumps Today; open/focus/viewed do not; ⌘1…⌘9 follow recency, not pins or folder order.
...
- **Recency list:** run `pnpm --filter @pi-gui/desktop run test:e2e:runner -- apps/desktop/tests/core/sidebar-recency.spec.ts`. Threads from mixed folders share Today / Last 7 Days / Last 30 Days / Older. Click does not bump. Send bumps Today. `Meta+1` / `Control+1` selects the most recently sent thread even if another thread is pinned. This is fixture-backed Electron proof.
