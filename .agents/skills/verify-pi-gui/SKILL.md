...
Open the actual `conversation.zip` or `restart.zip` from the printed directory with `pnpm exec playwright show-trace`. The settings smoke instead uses `change.zip` and `restart.zip`. Inspect screenshots/video as well as assertions. On Linux the launcher omits Playwright Electron `recordVideo` because it stalls `loadURL` (empty window URL, `webContents.isLoading` stays true); screenshots, traces, and stream samples remain required. Keep credentials outside artifacts shared with reviewers; trace source includes test code and prompts, so use synthetic prompts only.
