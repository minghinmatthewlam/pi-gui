...
For the remaining mapped surfaces (skills Try and aliases, pin/unpin, thread cap, worktrees, queued follow-ups) after the default conversation proof:
...
This uses the same explicit real-auth environment as conversation. It is not a substitute for send/stream proof.
...
`proof.spec.ts`, selected only by `--smoke`, clicks Settings sections, opens Skills and New thread, and verifies a preference after restart. `--maintenance` (`maintenance.spec.ts`) then drives the remaining mapped surfaces in a visible no-test-hook session: Skills Try and slash aliases, pin/unpin, Show more / Show less, Create permanent worktree plus Local/Worktree, a real-provider Enter-queue / modified-Enter steer, and pin/unpin while that run is still going. Neither recipe creates the default conversation Alpha/Bravo path through IPC or injects assistant events. The model configuration and scratch workspace are launch fixtures; they do not prove account onboarding or native folder opening.

Playwright sends input to the real renderer; it does not move the desktop pointer. Native pickers, clipboard, permissions and OS switching need a native/Computer Use journey. The launcher uses the development Electron binary with freshly built app code, not a packaged installation. Read the feature map for other required entry points. Native/package paths still have separate coverage; do not imply they passed with these proofs.
...
- `scripts/maintenance.spec.ts`: visible sweep of remaining mapped features (skills Try/aliases, pin including while running, thread cap, worktrees, queued follow-ups).
